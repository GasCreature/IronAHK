...
namespace IronAHK.Rusty
{
	partial class Core
	{
		...
		private static void GuiControlEdit(ref Control control, Form parent, string type, string options, string content)
		{
			string opts = null;
			...
			if (control is Button)
			{
				...
				if ((button.Tag as bool?) != true)
					...
					{
						SafeInvoke(Keyword_GuiButton + content);
					};
				else
					button.Tag = null;
			}
		...
		private static string[] GuiParseList(Control control, out int select, out bool clear)
		{
			select = 0;
			clear = false;
		...
		private static GuiInfo GuiAssociatedInfo(Control control)
		{
			return control.FindForm().Tag as GuiInfo;
		}

		private static void GuiControlMove(string mode, Control control)
		{
			if (mode.Length < 2)
				return;
		...
		private static Control GuiFindControl(string name)
		{
		...
		public static void GuiControl(string Command, string ControlID, string Param3)
		{
			...
			ctrl.Invoke((SimpleDelegate) delegate
			{
				GuiControlAsync(ctrl, Command, Param3);
			});
		}

		private static void GuiApplyExtendedStyles(Control control, string options)
		{
		}
