...
namespace IronAHK.Rusty
{
	partial class Core
	{
		private static string GuiId(ref string command)
		{
		...
		private static Icon GetIcon(string source, int n)
		{
		...
		private static void LV_RowOptions(ref ListViewItem row, string options)
		{
		...
		private static void LV_ColOptions(ref ColumnHeader col, string options)
		{
		}
		...
		private static TreeNode TV_FindNode(TreeView parent, long id)
		{
			var match = parent.Nodes.Find(id.ToString(), true);
			return match.Length == 0 ? null : match[0];
		}

		private static Bitmap GetScreen(Rectangle rect)
		{
