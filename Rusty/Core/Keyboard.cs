...
namespace IronAHK.Rusty
{
	partial class Core
	{
		...
		public static void Hotkey(string KeyName, string Label, string Options)
		{
			...
			try
			{
				key = Keyboard.HotkeyDefinition.Parse(KeyName);
			}
		...
		public static void Hotstring(string Sequence, string Label, string Options)
		{
			...
			var key = new Keyboard.HotstringDefinition(Sequence, proc)
			{
				Name = Sequence,
				Enabled = true,
		...
		public static void Input(out string result, string options, string endKeys, string matchList)
		{
			...
			if (inputHandler.IsBusy)
			{ // is there another Thread using this command already?
