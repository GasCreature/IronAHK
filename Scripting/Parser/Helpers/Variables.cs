...
namespace IronAHK.Scripting
{
	partial class Parser
	{
		...
		private string VarNormalisedName(string name)
		{
			return name.ToLowerInvariant();
		}

		private CodeArrayIndexerExpression VarId(string name)
		{
		...
		private CodeArrayIndexerExpression VarRef(params CodeExpression[] name)
		{
		...
		private CodeArrayIndexerExpression VarRef(string name)
		{
		...
		private CodeExpression VarIdOrConstant(string name)
		{
		...
		private CodeArrayIndexerExpression InternalVariable
		{
			get
			{
				return VarRef(string.Concat(Scope, ScopeVar + "\0", InternalID));
			}
		}
