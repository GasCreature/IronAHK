...
namespace IronAHK.Scripting
{
	...
	{
		private bool HasBody(MethodBase Base)
		{
			if (Base.IsAbstract)
				return false;
		...
		private void CopyMethodBody(MethodBase Base, MethodBuilder Builder)
		{
			if (HasBody(Base))
				CopyMethodBody(Base, Builder.GetILGenerator());
		}

		private void CopyMethodBody(MethodBase Base, ConstructorBuilder Builder)
		{
			if (HasBody(Base))
				CopyMethodBody(Base, Builder.GetILGenerator());
		}

		private void CopyMethodBody(MethodBase Base, ILGenerator Gen)
		{
		...
		private void CopyLocals(ILGenerator Gen, MethodBody Body)
		{
			foreach (LocalVariableInfo Info in Body.LocalVariables)
				Gen.DeclareLocal(GrabType(Info.LocalType), Info.IsPinned);
		}

		private void CopyOpcode(byte[] Bytes, ref int i, ILGenerator Gen, Module Origin, List<int> ExceptionTrinkets, Dictionary<int, Label[]> LabelOrigins)
		{
		...
		private bool TryReplaceBackingField(byte[] Bytes, int i, ILGenerator Gen, Module Origin)
		{
