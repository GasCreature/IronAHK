...
namespace IronAHK.Setup
{
	...
	{
		private static void Zip(string output, string paths, string working)
		{
		...
		private static void AdvZip(string output)
		{
			using (var adv = new Process())
			{
				adv.StartInfo = new ProcessStartInfo
				{
					FileName = "advzip",
					UseShellExecute = true,
					Arguments = string.Format("-z -4 \"{0}\"", output)
				};
		...
		private static void GnuZip(string output, string paths, string working)
		{
		...
		private static void SevenZip(string output, string paths, string working)
		{
