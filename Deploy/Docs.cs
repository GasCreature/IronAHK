...
namespace IronAHK.Setup
{
	...
	{
		private static void TransformDocs()
		{
			string root = string.Format("..{0}..{0}..{0}{1}{0}Site{0}docs{0}commands", Path.DirectorySeparatorChar, Name);

			var xsl = new XslCompiledTransform();

			using (var src = XmlReader.Create(Path.Combine(root, "view.xsl"), new XmlReaderSettings
			{
				ProhibitDtd = false
			}))
			{
...
<doc>
	...
	<members>
		...
		private static void RemoveCommonPaths(string parent, params string[] files)
		{
