...
namespace IronAHK.Setup
{
	...
	{
		private const string ExecFailed = "Cannot execute \"{0}\": {1}";
		...
		private static string Name
		{
			get
			{
				return typeof(Program).Namespace.Split(new[] { '.' }, 2)[0];
			}
		}
		...
		private static string Version
		{
			get
			{
				return IronAHK.Program.Version.ToString();
			}
		}
		...
		private static string WorkingDir
		{
			get
			{
				return Path.GetFullPath(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location));
			}
		}
